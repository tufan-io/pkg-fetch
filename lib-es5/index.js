...
function download(_a, local) {
    ...
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0:
                    url = "https://github.com/tufan-io/tufan-pkg-fetch/releases/download/" + tag + "/" + name;
                    _c.label = 1;
                case 1:
                    _c.trys.push([1, 4, , 5]);
                    return [4 /*yield*/, utils_1.downloadUrl(url, local)];
                case 2:
                    _c.sent();
                    return [4 /*yield*/, utils_1.plusx(local)];
                case 3:
                    _c.sent();
                    return [3 /*break*/, 5];
                case 4:
        ...
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0:
                    ...
                    if (!nodeRange)
                        throw log_1.wasReported('nodeRange not specified');
                    if (!platform)
                        throw log_1.wasReported('platform not specified');
                    if (!arch)
                        throw log_1.wasReported('arch not specified');
                    nodeRange = system_1.abiToNodeRange(nodeRange); // 'm48' -> 'node6'
                    if (!system_1.isValidNodeRange(nodeRange)) {
                        throw log_1.wasReported("nodeRange must start with 'node'");
                    }
                    if (nodeRange !== 'latest') {
                        nodeRange = "v" + nodeRange.slice(4); // 'node6' -> 'v6' for semver
                    }
                    platform = system_1.toFancyPlatform(platform); // win32 -> win
                    arch = system_1.toFancyArch(arch); // ia32 -> x86
                    nodeVersion = satisfyingNodeVersion();
                    if (!nodeVersion) {
                        throw log_1.wasReported("No available node version satisfies '" + opts.nodeRange + "'");
                    }
                    fetched = places_1.localPlace({
                        from: 'fetched',
                        arch: arch,
                ...
                case 6:
                    if (!dryRun && fetchFailed) {
                        ...
                        if (forceFetch) {
                            throw log_1.wasReported("Failed to fetch.");
                        }
                    }
                    if (!dryRun) {
                    ...
                    if (system_1.hostPlatform !== platform) {
                        if (system_1.hostPlatform !== 'alpine' || platform !== 'linuxstatic') {
                            throw log_1.wasReported("Not able to build for '" + opts.platform + "' here, only for '" + system_1.hostPlatform + "'");
                        }
                    }
                    if (system_1.knownArchs.indexOf(arch) < 0) {
                        throw log_1.wasReported("Unknown arch '" + opts.arch + "'. Specify " + system_1.knownArchs.join(', '));
                    }
                    if (dryRun) {
                        return [2 /*return*/, 'built'];
                    }
